...
function DiabloMod_AdjustOrbs()
	...
	--DiabloMod_BlueOrb:SetHeight(healthPercent*86);
	DiabloMod_RedOrb:SetHeight(healthPercent*169);
	DiabloMod_RedOrb:SetTexCoord(0, 1, 1-healthPercent, 1);
	
	if (healthPercent >= 0.5) then
		DiabloMod_RedOrb:SetVertexColor((1.6-1.6*healthPercent)+0.2, 1.0, 0.2); --rätt kanske
	else
		DiabloMod_RedOrb:SetVertexColor(1.0, 2.0*healthPercent, 0.2*healthPercent); --hoppas
	end
	...
	DiabloMod_BlueOrb:SetHeight(manaPercent*169);
...
function DiabloMod_OnEvent(event)
	...
	if (event=="UNIT_HEALTH") then 
		...
		DiabloMod_RedOrb:SetHeight(healthPercent*173);
		DiabloMod_RedOrb:SetTexCoord(0, 1, 1-healthPercent, 1);
		if (healthPercent >= 0.5) then
			DiabloMod_RedOrb:SetVertexColor((1.6-1.6*healthPercent)+0.2, 1.0, 0.2); --rätt kanske
		else
	...
	if (event=="UNIT_MANA" or event=="UNIT_RAGE" or event=="UNIT_ENERGY" or event=="UNIT_RUNIC_POWER" ) then    
		...
		DiabloMod_ManaText:SetText(UnitMana("player").." / ".. UnitManaMax("player"));
		--DiabloMod_BlueOrb:SetHeight(manaPercent*86);
		DiabloMod_BlueOrb:SetHeight(manaPercent*173);
		DiabloMod_BlueOrb:SetTexCoord(0, 1, 1-manaPercent, 1);
		return;
	end
end
